...
export default {
  ...
  methods: {
    ...
    async fetchdeletecertificate() {
      var prompStr = prompt(
        '수료증이 삭제되며 복구할 수 없습니다.\n삭제를 원하면 "삭제"를 입력해주세요.'
      );
      if (prompStr == null) {
        return;
      }
      if (prompStr == "삭제") {
        var deletecertificate = await RestAPIManager.API_deletecertificate(this.info.id);
        console.log(deletecertificate);
        if (deletecertificate.success === true) {
          alert("성공적으로 삭제되었습니다.");
          this.fetchCertificateData();
        } else {
          alert(deletecertificate.message);
        }
      } else {
        alert("정확하게 입력해주세요.");
        return;
